...
window.jTour = function (tourdata, options) {
    ...
    //short validation
    if (typeof jQuery !== 'function' || jQuery.fn.jquery.replace(/\./g, '') < 171) $.error('jQuery >1.7 is required!');
    ...
    var version = '1.0.0',
        ...
        timeout, steps, stepfunction, manualskip = false,
        scrollTop, scrollLeft, offsetX, offsetY, offsetFactor = 2.5,
        classStr, cssNames = {
            x: "scrollLeft",
            y: "scrollTop"
        },
    ...
    function init() {
        ...
        $(window).unbind('.jTour').bind('resize.jTour', setClientDimesions).resize();
    ...
    function continueTour() {
        ...
        //only continue if autoplay is true and no manual pause action was called
        if (isPaused && autoplay && !manualskip) {
            //clear the timout to prevent any further action
            clearTimeout(timeout);
    ...
    function show(step) {
        ...
        box[options.animationOut](options.delayOut, function () {
            ...
            //callback if it's not the first step
