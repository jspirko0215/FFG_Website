...
class Api2_model extends CI_Model
{
    ...
    public function getBestGame($memberId)
    {
        $this->db->where('memberID', $memberId);
        $this->db->where('isBestGame', 1);
        $res = $this->db->get_where('gamedetail')->result();
        return $res;
    }

    public function getLastGame($memberId)
    {
        $this->db->where('memberID', $memberId);
    ...
    {
        $data['gameDate'] = date('Y-m-d H:i:s');
        $this->db->insert('gamedetail', $data);
        return $data;
    }
