...
class Api2 extends API2_Controller
{
    ...
    public function getMemberBestGame()
    {
        $this->load->model('api2_model', '', TRUE);

        $memberId = $this->request['memberId'];
        $res = $this->api2_model->getBestGame($memberId);
        if ($res) {
            $outData = array();
            foreach ($res as $row) {
                $outData[] = array('lightTimeSeries' => $row->lightTimeSeries, 'gameCompleted' => $row->gameCompleted);
            }
            $this->response['best_game_set'] = $outData;
        } else {
            $this->addError('Data not found');
        }
    }

    public function getMemberLastGame()
    {
        $this->load->model('api2_model', '', TRUE);

        $memberId = $this->request['memberId'];
        $res = $this->api2_model->getLastGame($memberId);
        if ($res) {
            $outData = array();
            foreach ($res as $row) {
                $outData[] = array('lightTimeSeries' => $row->lightTimeSeries, 'gameCompleted' => $row->gameCompleted);
            }
            $this->response['last_game_set'] = $outData;
        } else {
            $this->addError('Data not found');
        }
    }
    ...
    public function logGame()
    {
        $this->load->model('api2_model', '', TRUE);
        $data=$this->request;
        if ($res = $this->api2_model->logGame($data))
        {
            $this->response['logGame'] = $this->transform($res);
        }
        else
        {
            $this->addError('Error');
